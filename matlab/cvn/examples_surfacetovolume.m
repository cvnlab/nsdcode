...
hemis = {'lh' 'rh'};
rois = {'Kastner2015' 'HCP_MMP1' 'visualsulc' 'nsdgeneral'};
for subjix=1:8
  for r=1:length(rois)
    for p=1:length(hemis)
      ...
      nsd_mapdata(subjix,'fsaverage',sprintf('%s.white',hemis{p}),sourcedata,[],[],outputfile,[],fsdir);
        % copy the .ctab file too:
      assert(copyfile(sprintf([nsd_datalocation '/freesurfer/fsaverage/label/%s.mgz.ctab'],rois{r}), ...
...
for subjix=1:8
  for r=1:length(rois)
    ...
    for p=1:length(hemis)
      ...
      assert(isequal(temp,0:max(temp)));
    ...
    % check that two hemispheres have the same max value (N)
    assert(max(data{1})==max(data{2}));
    N = max(data{1});
